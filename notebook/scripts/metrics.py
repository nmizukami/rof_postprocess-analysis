...
def corr(qsim, qobs):
    ...
    if qobs1.size <= 1: # correlation cannot be computed with size 1 array
        error_metric = np.nan
    elif len(np.unique(qsim1)) == 1 or len(np.unique(qobs1)) == 1: # if array has the same values, standard deviation become zero, causing "divide by zero" issue
        error_metric = np.nan
    else:
...
def alpha(qsim, qobs):
    qsim1, qobs1 = remove_nan(qsim, qobs)
    return math.sqrt(np.sum((qsim1-np.mean(qsim1))**2)/len(qsim1))/math.sqrt(np.sum((qobs1-np.mean(qobs1))**2)/len(qobs1))
...
def kge(qsim, qobs):
    qsim1, qobs1 = remove_nan(qsim, qobs)
    return 1-math.sqrt((1-corr(qsim1, qobs1))**2 + (alpha(qsim1, qobs1)-1)**2 + (beta(qsim1, qobs1)-1)**2)
